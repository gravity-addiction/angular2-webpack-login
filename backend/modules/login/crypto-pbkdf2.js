...
exports.hashPassword = function(password, callback) {
  ...
  crypto.randomBytes(process.env.crypt_saltBytes, function(err, salt) {
    if (err) {
      return callback(err);
    }

    var bSalt = salt.toString('base64').slice(0, process.env.crypt_saltBytes);
    crypto.pbkdf2(password, bSalt, process.env.crypt_iterations, process.env.crypt_hashBytes,
      function(err, hash) {
...
exports.verifyPassword = function(password, combined, callback) {
  ...
  crypto.pbkdf2(password, hash[1], parseInt(hash[2]), parseInt(hash[3]), function(err, verify) {
